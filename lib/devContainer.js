...
exportContainerRuntime(module, {
  ...
  getConfigDefaulter (configPath) {
    ...
    defaulter.set('statsOptions', 'call', this.parseConfigStatsOptions.bind(this))
    defaulter.set('statsToJsonOptions', 'call', this.parseConfigStatsToJsonOptions.bind(this))
    defaulter.set('statsToStringOptions', 'call', this.parseConfigStatsToStringOptions.bind(this))
    ...
    defaulter.set('jsonifyCompilerStats', function (stats) {
      return stats.toString(this.statsToJsonOptions)
    })
    ...
    defaulter.set('stringifyCompilerStats', function (stats) {
      return stats.toString(this.statsToStringOptions)
    })
  ...
  parseConfigStatsOptions (value, config) {
  ...
  parseConfigStatsToJsonOptions (value, config) {
    if (typeof value === 'boolean' || typeof value === 'string') {
      value = Stats.presetToOptions(value)
    }

    return Object.assign({source: false}, config.statsOptions, value)
  },
  parseConfigStatsToStringOptions (value, config) {
    if (typeof value === 'boolean' || typeof value === 'string') {
      value = Stats.presetToOptions(value)
    }

    return Object.assign({colors: supportsColor}, config.statsOptions, value)
  },
  ...
  shutUpContainer (proc, config) {
    Runtime.shutUpContainer.call(this, proc, config)

    this.debug('get webpack config')

    const webpackConfig = this.getWebpackConfig(config.webpackConfig)
    ...
    config.onCompilerWatchRun && compiler.plugin('watch-run', config.onCompilerWatchRun.bind(config))
    config.onCompilerBeforeCompile && compiler.plugin('before-compile', config.onCompilerBeforeCompile.bind(config))
    config.onCompilerCompile && compiler.plugin('compile', config.onCompilerCompile.bind(config))
    config.onCompilerThisCompilation && compiler.plugin('this-compilation', config.onCompilerThisCompilation.bind(config))
    config.onCompilerCompilation && compiler.plugin('compilation', config.onCompilerCompilation.bind(config))
    config.onCompilerMake && compiler.plugin('make', config.onCompilerMake.bind(config))
    config.onCompilerAfterCompile && compiler.plugin('after-compile', config.onCompilerAfterCompile.bind(config))
    config.onCompilerShouldEmit && compiler.plugin('should-emit', config.onCompilerShouldEmit.bind(config))
    config.onCompilerNeedAdditionalPass && compiler.plugin('need-additional-pass', config.onCompilerNeedAdditionalPass.bind(config))
    config.onCompilerEmit && compiler.plugin('emit', config.onCompilerEmit.bind(config))
    config.onCompilerAfterEmit && compiler.plugin('after-emit', config.onCompilerAfterEmit.bind(config))
    config.onCompilerDone && compiler.plugin('done', config.onCompilerDone.bind(config))
    config.onCompilerFailed && compiler.plugin('failed', config.onCompilerFailed.bind(config))
    config.onCompilerInvalid && compiler.plugin('invalid', config.onCompilerInvalid.bind(config))
    config.onCompilerWatchClose && compiler.plugin('watch-close', config.onCompilerWatchClose.bind(config))
  ...
  tryPrepareExtremeHMR (config, webpackConfig) {
    ...
    if (!autoRestart) {
      ...
      ensureConfigHasEntry(nodeConfigs, 'webpack/hot/poll', hmr.hotPollInterval, {topModuleEntries})
    }
    ...
    webConfigs.forEach((webConfig) => {
      ...
      ensureConfigHasEntry(webConfig, 'webpack-hot-middleware/client', hotMiddlewareOptions, {topModuleEntries})
    })
    ...
    nodeConfigs.forEach((nodeConfig) => {
      nodeConfig.plugins.push(function () {
        ...
        this.plugin(bundleAvailableEvent, (nodeBundle) => {
          decorateEventListener('request', nodeBundle, (listener) => {
      ...
      nodeConfig.plugins.push(function () {
        ...
        this.plugin(bundleAvailableEvent, (nodeBundle) => {
          webConfigs.forEach((webConfig, index) => {
            const publicPath = getOutputPublicPath(webConfig, {startsSlash: true, endsSlash: false})
