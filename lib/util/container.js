...
const Runtime = {
  ...
  async bootstrapContainer (proc, configPath) {
    ...
    await this.shutUpContainer(proc, config)
  ...
  getConfig (configPath) {
    let config = requireDefault(configPath, {cache: false})

    // Compatibility note:
    // * in webpack4, OptionsDefaulter's process() returns new options object
    // * in webpack3, OptionsDefaulter's process() mutates options object
    config = this.getConfigDefaulter(configPath).process(config) || config
  ...
  getConfigDefaulter (configPath) {
    ...
    defaulter.set('watchOptions', 'call', this.parseConfigWatchOptions.bind(this))
  ...
  getWatchpackOptions (container) {
    ...
    const watchDirs = this.getWatchpackOptionDirs(container)
    const watchFiles = this.getWatchpackOptionFiles(container)

    return Object.assign({}, container.config.watchOptions, {
      directories: preparePaths(watchDirs),
      files: preparePaths(watchFiles),
      fork: {
...
function assembleContainerRuntime (runtimePath, requireOptions) {
  const defaulter = new OptionsDefaulter()
  let runtime = requireDefault(runtimePath, requireOptions)
  ...
  // * in webpack4, OptionsDefaulter's process() returns new options object
  // * in webpack3, OptionsDefaulter's process() mutates options object
  runtime = defaulter.process(runtime) || runtime
